...
public class TabBase : ComponentBase
{
    ...
    // Lifecycle method triggered when parameters are set or changed - get case details, exhibits and tab content
    ...
    protected async Task OpenRichTextEditor()
    {
        ...
        DialogParameters<TabTextEditorDialog> parameters = new()
        {
            { p => p.CaseId, CaseId },
            { p => p.TabId, TabId },
            { p => p.SCase, SCase },
        ...
        IDialogReference dialog =
            await Dialog.ShowAsync<TabTextEditorDialog>("Rich text editor", parameters, options);
