...
public class LighthouseNotesAPIGet
{
    ...
    // GET: /users
    public async Task<(Pagination, List<User>)> Users(int page = 1, int pageSize = 10, string sort = "",
        bool sio = false, string search = "")
    {
        // Create request
    ...
    public async Task<string> ContemporaneousNote(string caseId, string noteId)
    {
        ...
        if (errorMessage.Title == "Could not find hash value for contemporaneous note!" ||
    ...
    public async Task<string> SharedContemporaneousNote(string caseId, string noteId)
    {
        ...
        // If error message is one about hashes display it
        if (errorMessage.Title == "Could not find hash value for shared contemporaneous note!" ||
            errorMessage.Title == "MD5 hash verification failed!" ||
            errorMessage.Title == "SHA256 hash verification failed!")
            return $"<span style=color:red> {errorMessage.Detail!} </span>";
    ...
    public async Task<string> SharedFile(string caseId, string type, string fileName)
    {
        ...
        // If error message is one about hashes display it
        if (errorMessage.Title == "Could not find hash value for the shared image!" ||
            errorMessage.Title == "MD5 hash verification failed!" ||
            errorMessage.Title == "SHA256 hash verification failed!")
            return $"/img/image-error.jpeg";
    ...
    // GET: /case/?/exhibits
    public async Task<(Pagination, List<Exhibit>)> Exhibits(string caseId, int page = 1, int pageSize = 10,
        string sort = "")
    {
        // Create request
